...
@router.post("")
async def create_project(payload: ProjectCreate, current_user: dict = Depends(require_role("OWNER", "ADMIN"))):
    ...
    project = execute(
        """
        INSERT INTO projects (tenant_id, name, key, description, created_by)
        VALUES (%s, %s, %s, %s, %s)
        RETURNING *
        """,
        (current_user["tenant_id"], payload.name.strip(), key, payload.description, current_user["id"]),
    )
...
async def update_project(project_id: str, payload: ProjectUpdate, current_user: dict = Depends(require_role("OWNER", "ADMIN"))):
    project = fetch_one("SELECT * FROM projects WHERE id = %s AND tenant_id = %s", (project_id, current_user["tenant_id"]))
    if not project:
        raise HTTPException(status_code=404, detail="Project not found")
    ...
    updated = execute(
        ...
        RETURNING *
        """,
        (name, description, status, project_id, current_user["tenant_id"]),
    )
...
async def archive_project(project_id: str, current_user: dict = Depends(require_role("OWNER", "ADMIN"))):
    updated = execute(
        "UPDATE projects SET status = 'ARCHIVED', updated_at = now() WHERE id = %s AND tenant_id = %s RETURNING *",
